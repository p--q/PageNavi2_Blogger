...
var PageNavi2_Blogger = PageNavi2_Blogger || function() {
    ...
    function redirect(pageNo) {  // ページ番号のボタンをクリックされた時に呼び出される関数。
        ...
        if (pageNo==1) {  // 1ページ目を取得するときはページ番号からURLを算出する必要がない。
            ...
            var startPost = vars.firstPagePosts + (vars.pageNo - 2) * vars.perPage;  // 新たに表示する先頭ページの先頭になる投稿番号を取得。1ページ目のみ投稿数が異なる。
            var url;
    ...
    function createNodes(total_posts) {  // 総投稿数からページナビのボタンを作成。
        ...
        var lastPageNo = 1 + Math.ceil((total_posts-vars.firstPagePosts) / vars.perPage); // 総投稿数から総ページ数を算出。1ページ目のみ投稿数が異なる。
